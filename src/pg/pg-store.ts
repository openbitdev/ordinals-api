...
export class PgStore extends BasePgStore {
  ...
  async getInscriptions(
    ...
    async getInscriptionsOnlyTx(
    page: DbInscriptionIndexPaging,
    filters?: DbInscriptionIndexFilters,
    sort?: DbInscriptionIndexOrder
  ): Promise<DbPaginatedResult<DbInscriptionOnlyTx>> {
    return await this.sqlTransaction(async sql => {
      const order = sort?.order === Order.asc ? sql`ASC` : sql`DESC`;
      let orderBy = sql`i.number ${order}`;
      switch (sort?.order_by) {
      ...
      const results = await sql<(DbInscriptionOnlyTx & { total: number })[]>`
       WITH results AS (
        SELECT
          i.ordinal_number AS sat_ordinal,
          cur.tx_index,
        ...
        FROM inscriptions AS i
        INNER JOIN current_locations AS cur ON cur.ordinal_number = i.ordinal_number
        WHERE cur.address IN ${sql(filters?.address || [])}
        ORDER BY ${orderBy}
        ...
        cur_l.value
        FROM results AS r
        INNER JOIN locations AS cur_l
        ON cur_l.ordinal_number = r.sat_ordinal
        AND cur_l.block_height = r.block_height
        AND cur_l.tx_index = r.tx_index
      `;
      return {
        total: total ?? results[0]?.total ?? 0,
        results: results ?? [],
