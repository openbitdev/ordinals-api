...
const IndexRoutes: FastifyPluginCallback<Record<never, never>, Server, TypeBoxTypeProvider> = (
  ...
  fastify.get(
    ...
    '/inscriptions/tx-only',
    {
      schema: {
        ...
        querystring: Type.Object({
          ...
          // Pagination
          offset: Type.Optional(OffsetParam),
          limit: Type.Optional(LimitParamOnlyTx),
          // Ordering
          order_by: Type.Optional(OrderByParam),
          order: Type.Optional(OrderParam),
        }),
        response: {
          200: PaginatedResponse(GetInscriptionsOnlyTxSchema, 'Not Paginated Inscriptions Response'),
          404: NotFoundResponse,
        },
      },
    },
    ...
    async (request, reply) => {
      const limit = request.query.limit ?? DEFAULT_API_LIMIT;
      const offset = request.query.offset ?? 0;
      const inscriptions = await fastify.db.getInscriptionsOnlyTx(
        { limit, offset },
        {
          ...blockParam(request.query.genesis_block, 'genesis_block'),
          ...blockParam(request.query.from_genesis_block_height, 'from_genesis_block'),
      ...
      await reply.send({
        ...
        offset,
        total: inscriptions.total,
        results: parseDbInscriptionOnlyTx(inscriptions.results),
      });
    }
  );
